namespace JanHafner.Smartbar.ProcessApplication.Commanding
{
    ...
    public sealed class UpdateProcessApplicationHotKeyCommandHandler : CommandHandler<UpdateProcessApplicationHotKeyCommand>
    {
        [NotNull]
        private readonly ISmartbarDbContext smartbarDbContext;

        [ImportingConstructor]
        public UpdateProcessApplicationHotKeyCommandHandler([NotNull] IEventAggregator eventAggregator,
        ...
        public override async Task<Boolean> CanHandleAsync(ICommand command)
        {
            return await base.CanHandleAsync(command) && this.smartbarDbContext.Groups.SelectMany(g => g.Applications).OfType<ProcessApplication>()
                        .Any(
                            application => application.Id == ((UpdateProcessApplicationHotKeyCommand) command).ApplicationId);
        }

        public override async Task HandleAsync(UpdateProcessApplicationHotKeyCommand command)
        {
            if (command == null)
            {
            ...
            updatedProcessApplication.UpdateHotKey(command.HotKeyModifier, command.HotKey);

            await this.smartbarDbContext.SaveChangesAsync();

            this.EventAggregator.GetEvent<ApplicationHotKeyUpdated>().Publish(command.ApplicationId);
